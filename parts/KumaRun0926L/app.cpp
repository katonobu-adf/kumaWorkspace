...
void racer_task(intptr_t exinf)
{
    ...
    if ( ev3_button_is_pressed(BACK_BUTTON) ) {
    	...
    	if (currState >2) {
    		...
    		int range = gNavigator->getPosture() / 100;
    		// int range = gNavigator->getVelocityMax() - gNavigator->getVelocityMin();
    		// int shock = gNavigator->shockDetectEx(200, 90) ? 1 : 0;
            int shock;
            shock = gNavigator->getStepCount();
            // if (gNavigator->shockDetectEx3(150, 100)) {
            //     shock = 1;
            // } else {
            //     shock = 0;
            // }
            gEv3Logger->logging(state, forward, turn, range, shock);
            // ADF)加藤
    	} 
