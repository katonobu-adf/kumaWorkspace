...
const int  Driver::GARAGE_IN_R =  8;   // ガレージイン（LRコース兼用）
// ADF)加藤
      ...
      mIsInitialized(false)
{
    ...
    step = new Step(   mNavigator, mBalancingWalker, mTail );
    mTask[7] = new GarageIn(   mNavigator, mBalancingWalker, mTail );
    garageInR = new GarageInR(   mNavigator, mBalancingWalker, mTail );
    // ADF)加藤
...
int Driver::run()
{
    ...
    switch(state){
        ...
        case GARAGE_IN:     // ガレージイン（Lコース）
            ...
            if (ret == 1) {
                garageInR->setActiveMode(10);
                // GarageInで20cm、残り距離80cm
                garageInR->setDistanceToGarage(800);
                mNavigator->setState(GARAGE_IN_R);
            }
            break;
        case GARAGE_IN_R:     // ガレージイン（LRコース兼用）
            ret = garageInR->run();
            break;
        // ADF)加藤
        default:
