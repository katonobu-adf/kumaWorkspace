...
bool Navigator::shockDetectEx2(int rangeThreshold, int inHardActionCountThreshold) {
    ...
    // inHardActionCountが閾値以上のカウントなら段差とみなす
    if (inHardActionCount > inHardActionCountThreshold && degSpeedAvg > rangeThreshold) {
        return true;
    }
    return false;
}
...
// 衝撃検出（別アルゴリズム）
bool Navigator::shockDetectEx3(int rangeThreshold, int inHardActionCountThreshold) {
    ...
    // 範囲が閾値以上かつ最大値が閾値以上
    // 想定だとrangeThreshold:150 inHardActionCountThreshold:100
    if (range > rangeThreshold && degSpeedMax > inHardActionCountThreshold) {
        return true;
    } else {
        return false;
    }
...
void Navigator::setStepCount(int stepCount) {
    Navigator::stepCount = stepCount;
}

int Navigator::getStepCount() {
    return stepCount;
}
int Navigator::getActiveMode() {
    return activeMode;
}

void Navigator::setActiveMode(int activeMode) {
    Navigator::activeMode = activeMode;
}

int Navigator::getGrayScale() {
    return mLineMonitor->getGrayScale(false);
}
