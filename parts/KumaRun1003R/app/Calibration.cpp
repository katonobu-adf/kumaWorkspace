...
const int Calibration::ADD_GRAY     = 35;   // 灰色の明るさに近づかせるため、黒の値に足す値
const int Calibration::DEF_AMBIENT  = 20;   // 基準とする環境光の値
...
void Calibration::dispAmbient(){
    ...
    // 0927:環境光でしきい値の調整をする
    int val = (ambient - DEF_AMBIENT) / 10;
    if(val <= 0){
        if(val == 0){
            ambient_suffering = 0;
        }else{
            ambient_suffering = 1 * abs(val);
        }

    }else{
        ambient_suffering = -1 * abs(val);
    }
    sprintf(buf, "AmbientSuff:%3d", ambient_suffering );
    ev3_lcd_draw_string( buf, 70, LINE_SPACING * 3);
}
...
void Calibration::confirmation(){
    ...
    threshold = threshold + ambient_suffering; // 環境光の影響によって調整する
