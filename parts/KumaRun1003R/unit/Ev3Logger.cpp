 ...
 * ロギングを行う
 */
void Ev3Logger::logging(int state, int forward, int turn, int active_mode, int grayScaleValue) {

    if (state == 8) {
        colorBrightnessValue = grayScaleValue;
    } else {
    ...
    sprintf(datas, "%ld,%d,%d,%d,%d,%ld,%ld,%ld,%d,%d,%d,%d,%d\r\n", 
        time, 
